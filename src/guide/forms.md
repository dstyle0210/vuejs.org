...
`debounce` 매개 변수는 사용자 입력 이벤트를 `디바운스`하지 않는 점에 유의하십시오 : 이것은 기초가되는 데이터에 "쓰기"작업을 하는 경우에 사용합니다. 따라서 `debounce`를 사용하면 데이터 변경에 반응하기 위해 `vm.$watch()`를 사용해야합니다. 진짜 DOM 이벤트를 디바운스하려면 [debounce filter](/api/#debounce)를 사용합니다.
